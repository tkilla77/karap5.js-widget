...
export const Directions = createDirections();
...
export class Kara {
  constructor(grid, coords = Coordinates.fromXy(1, 1), direction = Directions.RIGHT, sounds = Sounds.noSounds()) {
    this.grid = grid;
  ...
  turnLeft() {
    this.sounds.turnLeft();
    this.direction = this.direction.left;
    this.sounds.afterMove(this);
  }
  turnRight() {
    this.sounds.turnRight();
...
class Sounds {
  ...
  /** Returns a Sounds instance that makes a success sound if the 
   * beetle has a mushroom in front, plus movement sounds. */
  static mushroomGoal() {
    let sounds = new Sounds();
    sounds.preload = () => {
      try {
        soundFormats('mp3', 'ogg');
        sounds.stepSound = loadSound('static/sounds/step2.mp3');
        sounds.turnSound = loadSound('static/sounds/turn.mp3');
        sounds.successSound = loadSound('static/sounds/goal.mp3');
        sounds.boingSound = loadSound('static/sounds/boing2.mp3');
      } catch (e) {
        console.log(e);
      }
    };
    ...
    sounds.turnLeft = () => {
      if (sounds.turnSound) {
        sounds.turnSound.play();
      }
    };
    sounds.turnRight = sounds.turnLeft;
    sounds.afterMove = (kara) => {
      if (sounds.successSound && kara.mushroomFront()) {
        sounds.successSound.play();
      }
    };
    sounds.boing = () => {
      if (sounds.boingSound) {
        sounds.boingSound.play();
      }
    }
    return sounds;
  }
...
KaraWorld.create(
  ...
  static create(gridspec = EMPTY_GRID_SPEC, cell_size = 25, install = true, keyhandler = true) {
    ...
    return this.fromOptions(gridspec, options);
  }
...
KaraWorld.create(
    ...
    * @param {string} [gridspec=EMPTY_GRID_SPEC] the world specification (see   
    *                 above)
    * @param {options} [KaraWorld.DefaultOptions] Karaworld options
    * @returns {KaraWorld} the created KaraWorld instance
    */
  static fromOptions(gridspec = EMPTY_GRID_SPEC, options = KaraWorld.DefaultOptions) {
    options = Object.assign({}, KaraWorld.DefaultOptions, options);
    let game = KaraWorld.fromStringSpec(gridspec);
    if (options.install) {
      const cell_size = options.cell_size;
      let size = game.grid.gridSize();
      if (false && options.success_handler) {
        let sounds = Sounds.mushroomGoal();
        window.preload = () => {
          try {
            sounds.preload();
          } catch (e) {
        ...
        game.kara.sounds = sounds;
      }
      window.setup = () => {
        createCanvas(size.x * cell_size, size.y * cell_size);
        game.executeKara(options.delay_ms, options.client_function, options.max_commands);
      };
      window.draw = () => {
        game.draw(cell_size);
      };
  ...
  constructor(grid, kara) {
    ...
    karaCopy.sounds = Sounds.noSounds();
  ...
  draw(cell_size) {
    push();
    if (this.options.success_handler) {
      if (this.options.success_handler.mushroomFront) {
        if (this.kara.mushroomFront()) {
          background(this.options.success_handler.background);
        }
      }
    }
...
KaraWorld.DefaultOptions = Object.freeze({
  install: true,
  keyhandler: true,
  cell_size: 25,
  delay_ms: 500,
